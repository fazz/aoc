...
for p in g:
    ...
    if p == 46:
        ...
        for pp in ((y-1, x), (y+1, x), (y, x-1), (y, x+1)):
            ...
            if x == 14 and y == 13:
                print(area[pp[0]][pp[1]].count)
            area[pp[0]][pp[1]].count += 1
...
print(
    sum([s.score for s in
        filter(lambda x: x.count == 4 and x.type in ('<', '>', '^', 'v', '#'), [spot for m in [area[y] for y in area] for spot in m.values()])
    ])
