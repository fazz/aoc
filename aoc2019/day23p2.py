...
while True:
    ...
    if k == 'OUT':
        ...
        v = idles.setdefault(active, 0)
        idles[active] = v + 1

    if len(list(filter(lambda x: x[1] > 1, idles.items()))) == count and natstore is not None:
        print("Idle detected")

        natbuffer.append(natstore)
        if len(natbuffer) > 2:
            natbuffer.pop(0)
            if natbuffer[0][1] == natbuffer[1][1]:
                result = natbuffer[1][1]
                break
