...
xbase = min([x[0] for x in area])
ybase = min([x[1] for x in area])

xend = max([x[0] for x in area])
yend = max([x[1] for x in area])

print("")

for y in range(yend, ybase-1, -1):
    for x in range(xbase, xend+1):
        if (x,y) in area and area[(x,y)] == 1:
            print('#', end = '')
        else:
            print(' ', end = '')
    print("")
